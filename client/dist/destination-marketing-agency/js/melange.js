...
(function initAnchoredScroll() {
  ...
  function resolveScrollEl(id) {
    if (id === "report-access") {
      return (
        document.getElementById("report-form-mount") ||
        document.querySelector(".report-access-shell") ||
        document.getElementById("report-access")
      );
    }
    return document.getElementById(id);
  }
...
(function initNetworkSphere() {
  ...
  function run() {
    var roots = document.querySelectorAll("[data-network-sphere]");
    if (!roots.length) return;
    if (typeof window.__initAboutNetworkSphere === "function") {
      roots.forEach(window.__initAboutNetworkSphere);
      return;
    }
