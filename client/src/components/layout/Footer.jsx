...
import Insights from "../RevamperHome/Insights";
import {
  blog261Banner,
  blog262Banner,
  blog263Banner,
...
const Footer = ({ customInsights }) => {
  ...
  const defaultInsights = [
    {
      title: "The rise of creator storefronts and how they are reshaping brand-influencer partnerships",
    ...
    {
      title: "Sustainable design and packaging: why 2025 consumers judge before they click buy",
    ...
    {
      title: "Community First Content Stacks: Why Owning Your Audience Is The New Moat",
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const blogsQuery = query(collection(db, "blogs"), limit(3));
        const blogsSnapshot = await getDocs(blogsQuery);
        const fetchedBlogs = blogsSnapshot.docs.map(doc => {
          const data = doc.data();
          const cat = data.categories?.length > 0 ? data.categories[0] : (data.category || "");
          const formattedCat = cat.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
          return {
            title: data.title,
            date: `${formattedCat} - ${data.date}`,
            image: data.image,
        ...
        setDynamicInsights(fetchedBlogs.slice(0, 3));
      } catch (err) {
        console.error("Error fetching global insights data:", err);
      }
    };
    fetchData();
  }, []);
  ...
  return (
    <>
      <Insights insightsData={customInsights?.length > 0 ? customInsights : (dynamicInsights.length > 0 ? dynamicInsights : defaultInsights)} />
