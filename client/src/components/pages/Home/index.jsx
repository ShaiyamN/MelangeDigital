...
import ReportLeadForm from "../../layout/ReportLeadForm";
import {
  readReportParam,
  storePendingReport,
} from "../../../constants/reportLead";
...
/**
 * Marketing home at `/`: React shell + extracted markup + assets from public/destination-marketing-agency/.
 */
const Home = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancelled) console.error("[Home]", err);
      }
    })();
  ...
  useEffect(() => {
    const slug = readReportParam();
    if (slug) storePendingReport(slug);
  }, []);

  useEffect(() => {
    if (!cssReady || !rootRef.current) return;
    setFormMount(rootRef.current.querySelector("#report-form-mount"));
  }, [cssReady]);
  ...
  return (
    <>
      <Helmet>
        ...
        <link
          href="https://fonts.googleapis.com/css2?family=Alan+Sans:wght@400;500;600;700;800&family=Baskervville:ital@1&family=Lato:wght@400;700&family=Poppins:ital,wght@0,400;0,500;0,600;0,700;1,400&display=swap"
          rel="stylesheet"
        />
      ...
      />
      {formMount ? createPortal(<ReportLeadForm />, formMount) : null}
      <Footer />
    </>
  );
};
